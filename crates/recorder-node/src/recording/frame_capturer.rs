//! Frame capture and AI integration for active recordings
//!
//! This module handles periodic frame extraction from active recordings
...
impl Default for FrameCaptureConfig {
    fn default() -> Self {
        Self {
            ...
            capture_interval_secs: 2, // Slightly slower for recordings
            frame_width: 640,
...
/// Start frame capture loop for a recording
///
/// This spawns a background task that periodically extracts frames from the recording source
...
pub fn start_frame_capture(
    ...
    tokio::spawn(async move {
        info!(
            ...
            "starting recording frame capture loop"
        );
        ...
        loop {
            tokio::select! {
                _ = cancel_token.cancelled() => {
                    info!(recording_id = %recording_id, "recording frame capture cancelled");
                    break;
                }
                _ = interval.tick() => {
                    frame_seq += 1;

                    // Extract frame from recording source
                    match frame_extractor::extract_frame_jpeg(
                        ...
                        Err(e) => {
                            error!(
                                ...
                                frame_seq = frame_seq,
                                error = %e,
                                "failed to extract frame from recording"
                            );
                        }
                    }
                }
        ...
        info!(recording_id = %recording_id, total_frames = frame_seq, "recording frame capture stopped");
    });
}
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.capture_interval_secs, 2);
