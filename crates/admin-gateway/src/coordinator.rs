...
use common::leases::{
  ...
  LeaseRenewRequest, LeaseRenewResponse,
};
use reqwest::Url;
  ...
  async fn acquire(&self, request: &LeaseAcquireRequest) -> Result<LeaseAcquireResponse> {
    ...
    Ok(
      resp
        .json()
        .await
        .context("failed to parse acquire response")?,
    )
  }

  #[instrument(skip_all, fields(lease = %request.lease_id))]
  async fn renew(&self, request: &LeaseRenewRequest) -> Result<LeaseRenewResponse> {
    let url = self.endpoint("v1/leases/renew")?;
    let resp = self
      .client
      .post(url)
      .json(request)
      .send()
      .await
      .context("coordinator renew request failed")?;
    let resp = resp
      .error_for_status()
      .context("coordinator renew returned error status")?;
    Ok(
      resp
        .json()
        .await
        .context("failed to parse renew response")?,
    )
  }
