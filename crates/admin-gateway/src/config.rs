...
impl GatewayConfig {
  pub fn from_env() -> Result<Self> {
    let bind = env::var("ADMIN_GATEWAY_ADDR").unwrap_or_else(|_| "0.0.0.0:8081".to_string());
    let bind_addr: SocketAddr = bind.parse().context("invalid ADMIN_GATEWAY_ADDR")?;

    let coord =
      env::var("COORDINATOR_ENDPOINT").unwrap_or_else(|_| "http://127.0.0.1:8082".to_string());
