...
async fn fetch_recording_stats(state: &AppState) -> anyhow::Result<RecordingStats> {
    ...
    if response.status().is_success() {
        ...
        let today = 0; // TODO: Filter by date
        let total_size_bytes = 0; // TODO: Sum file sizes
...
async fn fetch_ai_task_stats(state: &AppState) -> anyhow::Result<AiTaskStats> {
    ...
    if response.status().is_success() {
        let tasks: Vec<Value> = response.json().await?;
        let total = tasks.len();
        let active = tasks
            .iter()
            .filter(|t| t["status"] == "running")
            .count();
        let detections_today = 0; // TODO: Implement
...
async fn fetch_alert_stats(state: &AppState) -> anyhow::Result<AlertStats> {
    ...
    if response.status().is_success() {
        let rules: Vec<Value> = response.json().await?;
        let active_rules = rules.iter().filter(|r| r["enabled"] == true).count();
        let alerts_today = 0; // TODO: Implement
